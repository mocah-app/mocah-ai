...
export function OrganizationProvider({ children }: { children: ReactNode }) {
  ...
  const loadOrganizations = async () => {
    ...
    try {
      ...
      if (session?.session?.activeOrganizationId) {
        const activeOrg = orgs?.find((o: Organization) => o.id === session.session.activeOrganizationId);
  ...
  const createOrganization = async (data: CreateOrgInput): Promise<Organization | null> => {
    try {
  ...
  const updateOrganization = async (orgId: string, data: UpdateOrgInput): Promise<void> => {
    try {
      ...
      logger.debug('Updating organization', {
        ...
        metadataKeys: updatePayload.metadata ? Object.keys(updatePayload.metadata) : [],
